...
echo
echo "Next steps:"
echo "  1. Create the superuser. Preferred: open the one-time installer URL that"
echo "     PocketBase logged on first start (no password touches any shell):"
echo "       pct exec $CTID -- journalctl -u pocketbase --no-pager | grep -o 'http[^\"]*pbinstal[^\"]*' | tail -1"
echo "     (empty? restart to re-log it: pct exec $CTID -- systemctl restart pocketbase)"
echo "     Open it in your browser with the host swapped for your domain/hosts entry."
echo "     Alternative via CLI — password is a required ARGUMENT, so read it silently"
echo "     first instead of typing it into the command line:"
echo "       read -rs -p 'superuser password: ' PBPASS; echo"
echo "       pct exec $CTID -- runuser -u pocketbase -- /usr/local/bin/pocketbase \\"
echo "         superuser upsert <email> \"\$PBPASS\" --dir $PB_DATA_DIR/pb_data; unset PBPASS"
if [ "$TLS_INTERNAL" = "1" ]; then
